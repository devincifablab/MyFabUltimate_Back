...
async function getUserAuth(app, userId) {
    ...
    const resTestIfCorrelationExist = await app.executeQuery(app.db, "SELECT " + columnToSee + " FROM `rolescorrelation` INNER JOIN gd_roles ON rolescorrelation.i_idRole = gd_roles.i_id WHERE rolescorrelation.i_idUser = ?", [userId]);
    // Error with the sql request
