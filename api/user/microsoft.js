    ...
    app.get("/api/user/login/microsoft", async function (req, res) {
        try {
            ...
            await axios({
                ...
                if(response.data.value[0].displayName != "De Vinci"){
                    res.sendStatus(401);
                }
            })
            await axios({
                ...
                const dbRes = await app.executeQuery(app.db, "SELECT `i_id` AS 'id' FROM `users` WHERE `v_email` = ?;", [email]);
                ...
                if (dbRes[1].length < 1) {
                    ...
                    const resInsertNewAccount = await app.executeQuery(app.db, "INSERT INTO `users` (`v_firstName`, `v_lastName`, `v_email`, `v_password`, `v_language`, `v_title`, `b_isMicrosoft`, `b_mailValidated`) VALUES (?, ?, ?, ?, ?, ?, ?, ?);", [firstName, lastName, email, sha256(token), language, jobTitle, 1, 1]);
                    // Error with the sql request
